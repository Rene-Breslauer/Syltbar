    ...
    document.querySelectorAll(".cart_total_shipping_price").forEach((item) => {
      if (item.dataset.shipping) {
        ...
        if (currentTotal < freeMoneyDelivery) {
          ...
          item
            .querySelector(".cart_qualifies_free_shipping_html")
            .classList.add("hidden");
          item
            .querySelector(".cart_no_qualifies_free_shipping_html")
  ...
  var params = {
    ...
    success: function (cart) {
      jQuery.getJSON("/cart.js", function (cart, textStatus) {
        ...
        } else jQuery(".cart-counter").removeClass("hidden");
      });
...
function on_change_gift(line_key, id_item, view) {
  if (jQuery("#Linegift_" + id_item).prop("checked") == true) {
    ...
    jQuery("#LinePriceList_" + id_item + ' input[type="checkbox"]').prop(
      "checked",
      false
    );
  } else {
